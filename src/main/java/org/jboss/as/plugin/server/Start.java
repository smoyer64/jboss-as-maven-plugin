...
public class Start extends AbstractServerConnection {
    ...
    public static final String JBOSS_DIR = "jboss-as";
    ...
    /**
     * The JBoss Application Server's home directory. If not used, JBoss
     * Application Server will be downloaded.
     */
     ...
     */
    @Parameter(defaultValue = "${project.build.directory}/", property = "jboss-as.targetDir")
	...
	if (!jbossHome.isDirectory()) {
	    throw new MojoExecutionException(String.format(
		    "JBOSS_HOME '%s' is not a valid directory.", jbossHome));
	}
	// JVM arguments should be space delimited
	final String[] jvmArgs = (this.jvmArgs == null ? null : this.jvmArgs
	        .split("\\s+"));
	...
	final ServerInfo serverInfo = ServerInfo
	        .of(this, javaHome, jbossHome, modulesPath, bundlesPath,
	                jvmArgs, serverConfig, startupTimeout);
	if (!serverInfo.getModulesDir().isDirectory()) {
	    throw new MojoExecutionException(String.format(
		    "Modules path '%s' is not a valid directory.", modulesPath));
	}
	if (!serverInfo.getBundlesDir().isDirectory()) {
	    throw new MojoExecutionException(String.format(
		    "Bundles path '%s' is not a valid directory.", bundlesPath));
	}
	...
	try {
	    ...
	    if (!server.isRunning()) {
		throw new MojoExecutionException("The server is not running.");
	    }
    ...
    private File extractIfRequired(final File buildDir)
	...
	final String jbossAsArtifact = String.format(
	        "org.jboss.as:jboss-as-dist:zip:%s", version);
	request.setArtifact(new DefaultArtifact(jbossAsArtifact));
	request.setRepositories(remoteRepos);
	getLog().info(
	        String.format("Resolving artifact %s from %s", jbossAsArtifact,
	                remoteRepos));
	...
	try {
		...
		final File extractTarget = new File(target.getAbsolutePath(),
		        entry.getName());
	...
	return new File(target.getAbsoluteFile(), String.format("jboss-as-%s",
	        version));
    }
